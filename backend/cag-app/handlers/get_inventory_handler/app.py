...
def query_items(dt_dict: Dict):
    (FilterExpression, ExpressionAttributeNames, ExpressionAttributeValues) = (
        dt_query_builder(dt_dict=dt_dict)
    )
    FilterExpression = FilterExpression
    ExpressionAttributeNames = ExpressionAttributeNames
    ExpressionAttributeValues = ExpressionAttributeValues

    item_list = table.query(
    ...
    return {"items": item_list, "total_price": total_price}
...
def lambda_handler(event, context):
    ...
    if event["httpMethod"] == "GET":
        ...
        if (
            ...
            and "next_key" in event["queryStringParameters"]
        ):
            next_key = event["queryStringParameters"]["next_key"]
            query_params["ExclusiveStartKey"] = {"id": next_key}

        item_resp = table.scan(**query_params)
        print(item_resp)
        item_list = item_resp["Items"]
        next_key = item_resp.get("LastEvaluatedKey", None)
        ...
        return {
            ...
            "body": json.dumps(
                {"items": item_list, "next_key": next_key},
                cls=DecimalEncoder,
            ),
        }
